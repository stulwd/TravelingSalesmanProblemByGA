...
class Solution(object):
    ...
    def fitness(self):
        for ind in self.individuals:
            ...
            timeCost = 0
            preCity = 0
            timeCost += self.cityData[ind.chromosome[preCity]][3]
            for i in range(1, 1 + (len(ind.chromosome) - 1)):
                distance += math.dist(self.cityData[ind.chromosome[preCity]][:2], self.cityData[ind.chromosome[i]][:2])
                currentTime = distance / 1
                readyTime = self.cityData[ind.chromosome[i]][3]
                dueTime = self.cityData[ind.chromosome[i]][4]
                if currentTime < readyTime:
                    timeCost += readyTime - currentTime
                elif currentTime > dueTime:
                    timeCost += currentTime - dueTime

                preCity = i
            ind.fitness = 1.0 / (distance + timeCost)
