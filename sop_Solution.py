...
class Solution(object):
    def __init__(self):
        ...
        self.__chromosome_length = 10
        self.__max_generation = 5000
        self.individuals = []
        self.eliteTrend = []
    ...
    def initialize(self):
        for i in range(self.__pop_size):
            while True:
                ...
                if self.SOP_check(ind) == True:
                    self.individuals.append(ind)
                    break

    def SOP_check(self, ind):
        index = -1
        for i in ind.chromosome:
            index += 1
            before = self.cityData[i][2]
            if before == 0:
                continue
            exist = False
            for j in range(index):
                if ind.chromosome[j] == int(before):
                    exist = True
                    break
            if not exist:
                return False
    ...
    def mutation(self):
        ...
        if random.random() < self.__mutation_rate:
            while True:
                ...
                if self.SOP_check(self.individuals[sel1]):
                    return
                else:
                    tmp = self.individuals[sel1].chromosome[mutPoint1]
                    self.individuals[sel1].chromosome[mutPoint1] = self.individuals[sel1].chromosome[mutPoint2]
